  ...
  return (
    <div className="min-h-screen bg-background-light dark:bg-background-dark text-slate-900 dark:text-gray-200 font-sans transition-colors duration-300">
      <main>
        ...
        <section className="relative pt-32 pb-20 md:pt-40 md:pb-24">
          <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 text-center">
            ...
            <div className="mt-8 flex flex-col sm:flex-row items-center justify-center gap-4">
              <Link href="/find-jobs" className="w-full sm:w-auto bg-primary text-white font-semibold px-8 py-3 rounded-lg hover:bg-primary/90 transition-colors text-center">
                Find Jobs
              </Link>
        ...
        <Suspense fallback={<CategoriesLoadingState />}>
          <CategoriesSection />
        </Suspense>
          ...
          <div className="grid md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="mt-4 space-y-2 text-sm">
                <li><Link href="/jobs" className="hover:text-primary dark:hover:text-primary transition">Browse Jobs</Link></li>
                <li><Link href="/freelance" className="hover:text-primary dark:hover:text-primary transition">Find Freelancers</Link></li>
                <li><Link href="/about" className="hover:text-primary dark:hover:text-primary transition">About Us</Link></li>
            ...
            <div>
              ...
              <div className="flex items-center space-x-4 mt-4">
                <a href="#" className="hover:text-primary dark:hover:text-primary transition">
...
const categoryIcons = [Code, Palette, Megaphone, FileText, Headphones] as const;
    ...
    <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
      ...
      <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
        {Array.from({ length: 5 }).map((_, index) => (
          <div
            key={index}
            className="bg-white/5 dark:bg-gray-800/60 p-6 rounded-xl border border-gray-200/10 dark:border-gray-700/50 animate-pulse"
          >
            <div className="w-12 h-12 mx-auto rounded-lg bg-slate-200/60 dark:bg-gray-700/80" />
            <div className="mt-4 h-5 rounded bg-slate-200/60 dark:bg-gray-700/80" />
            <div className="mt-2 h-4 rounded bg-slate-200/60 dark:bg-gray-700/80" />
          </div>
        ))}
      </div>
    </div>
...
const CategoriesSection = async () => {
  const categoriesResult = await handleGetAllCategories();
      ...
      <div className="container mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        {!categoriesResult.success ? (
          <div className="mt-12 text-center text-slate-600 dark:text-gray-400">
            {categoriesResult.message || "Unable to load categories right now."}
          </div>
          ...
          <div className="mt-12 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-6">
            {categoriesResult.data.map((category, index) => {
              ...
              return (
                <Link
                  key={category._id}
                  href={`/jobs?jobCategory=${encodeURIComponent(category._id)}`}
                  className="block bg-white/5 dark:bg-gray-800/60 p-6 rounded-xl border border-gray-200/10 dark:border-gray-700/50 text-center hover:border-primary transition-colors cursor-pointer"
                >
                  ...
                  <p className="text-sm text-slate-600 dark:text-gray-400 mt-1">
                    {category.description?.trim() || "Explore opportunities in this category."}
                  </p>
                </Link>
              );
            })}
